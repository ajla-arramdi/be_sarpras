    ...
    <div class="flex justify-between items-center mb-8 border-b border-teal-300 pb-4">
        <div class="flex items-center space-x-3">
            <div class="w-12 h-12 bg-teal-100 rounded-xl flex items-center justify-center">
                <i class="fas fa-box text-2xl text-teal-700"></i>
            </div>
            <div>
                <h2 class="text-3xl font-extrabold text-teal-900 tracking-tight">Daftar Barang</h2>
                <p class="text-sm text-teal-700">Total {{ $barangs->count() }} barang</p>
            </div>
        </div>
        ...
        <table class="min-w-full text-gray-800 border-collapse">
            <thead class="bg-teal-50">
                <tr>
                    ...
                    <th class="px-8 py-4 border-b border-teal-300 font-semibold text-teal-700 text-left tracking-wide text-sm">Foto</th>
                    <th class="px-8 py-4 border-b border-teal-300 font-semibold text-teal-700 text-left tracking-wide text-sm">Nama Barang</th>
                    <th class="px-8 py-4 border-b border-teal-300 font-semibold text-teal-700 text-left tracking-wide text-sm">Kode</th>
                    <th class="px-8 py-4 border-b border-teal-300 font-semibold text-teal-700 text-left tracking-wide text-sm">Jumlah</th>
                    <th class="px-8 py-4 border-b border-teal-300 font-semibold text-teal-700 text-left tracking-wide text-sm">Kategori</th>
                    <th class="px-8 py-4 border-b border-teal-300 font-semibold text-teal-700 text-left tracking-wide text-sm">Status</th>
            ...
            <tbody>
                @forelse($barangs as $barang)
                    <tr class="hover:bg-teal-100 transition cursor-pointer">
                        <td class="px-8 py-4 border-b border-teal-200">#{{ $barang->id }}</td>
                        <td class="px-8 py-4 border-b border-teal-200">
                            @if ($barang->foto)
                                <div class="relative w-10 h-10 rounded-lg overflow-hidden shadow-sm">
                                    <img src="{{ asset('storage/' . $barang->foto) }}" alt="Foto {{ $barang->nama_barang }}" class="w-full h-full object-cover" />
                        ...
                        <td class="px-8 py-4 border-b border-teal-200">
                            <div class="flex items-center">
                                ...
                                <div class="text-sm font-semibold text-teal-900">{{ $barang->nama_barang }}</div>
                            </div>
                        </td>
                        <td class="px-8 py-4 border-b border-teal-200">{{ $barang->code }}</td>
                        <td class="px-8 py-4 border-b border-teal-200">
                        ...
                        <td class="px-8 py-4 border-b border-teal-200">
                            <span class="px-3 py-1 text-sm rounded-full bg-teal-100 text-teal-800">{{ $barang->kategori->nama ?? '-' }}</span>
                        </td>
                        <td class="px-8 py-4 border-b border-teal-200">
                            @php
                        ...
                        <td class="px-8 py-4 border-b border-teal-200 text-center space-x-4 whitespace-nowrap">
                            ...
                            <form action="{{ route('barang.destroy', $barang->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus barang ini?')" class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit" 
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-8 py-20 text-center italic text-gray-400">
                            <div class="flex flex-col items-center justify-center space-y-3">
                                <i class="fas fa-boxes text-6xl text-teal-400"></i>
                                <p class="text-2xl font-semibold">Tidak ada barang tersedia</p>
                                <p class="text-sm text-gray-500">Silakan tambahkan barang baru</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
